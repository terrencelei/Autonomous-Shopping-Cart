...
4. **Lost → search (`Searcher`).** If the shopper leaves the frame, after a `SEARCH_GRACE_S` grace period (so a one-frame dropout doesn't trigger it) the cart rotates toward the side it was **last seen** to bring them back into view — a breakaway kick (`SPIN_KICK_RPM`, released after `KICK_TICKS`) to beat stall, then a steady sweep at `SEARCH_RPM`. It stops the instant a target reappears, and gives up (holds still, `Mode: LOST`) after sweeping `SEARCH_MAX_DEG` or `SEARCH_TIMEOUT_S`. Non-blocking like the spin.
...
The reused Cart View / World Map windows show `follow.py`'s own state in the **Mode** label each tick — `FOLLOW`, `SPIN`, `KICK`, `STOP`, `SEARCH`, or `LOST` — not the A\* planner's state machine. (The cart pose and dots on the World Map are still driven by the planner's odometry, which `follow.py` doesn't update, so treat those as decorative here.)
